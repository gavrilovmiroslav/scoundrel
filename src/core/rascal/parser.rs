...
lazy_static! {
    static ref EXPR_PREC_CLIMBER: PrecClimber<Rule> = {
        PrecClimber::new(vec![
            Operator::new(Rule::and_op, Left) | Operator::new(Rule::or_op, Left),
            ...
            Operator::new(Rule::plus_op, Left) | Operator::new(Rule::minus_op, Left),
...
impl ComponentSignature {
    ...
    pub fn new(name: &str, members: Vec<(&str, DataType)>) -> Self {
        ...
        Self {
            ...
            size,
            ptrs,
            members: ComponentSignature::lift(&members),
        }
    }
}
        ...
        let members: Vec<String> = self
            ...
            .map(|(p, id)| {
                format!(
                    "[{}-{}] {}: {:?}",
                    p,
                    self.members[id].typ.size_in_bytes() + p,
                    id,
                    self.members[id].typ
                )
            })
...
impl ComponentCallSite {
    pub fn negated(self) -> Self {
        ...
        let args = self.args;
        ComponentCallSite {
            modifier: ComponentModifier::Not,
            is_owned,
            owner,
            name,
            args,
    ...
    pub fn toggled(self) -> Self {
        ...
        let args = self.args;
        ComponentCallSite {
            modifier: ComponentModifier::Toggle,
            is_owned,
            owner,
            name,
            args,
...
pub enum RascalStatement {
    ...
    Match(
        RascalIdentifier,
        Vec<(RascalExpression, RascalBlock)>,
        Option<RascalBlock>,
        Option<RascalBlock>,
    ),
    ...
    ForLoopStatement(
        RascalIdentifier,
        (RascalExpression, RascalExpression),
        RascalBlock,
    ),
...
impl SystemSignature {
    ...
    fn parse_array_accessor(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        if let Some(index) =
            Self::parse_expression(TokenType::NonTerm(child.next().unwrap().into_inner()))
        {
    ...
    fn parse_negated_primary(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        if let Some(exp) =
            SystemSignature::parse_expression(TokenType::NonTerm(negated.clone().into_inner()))
        {
    ...
    fn parse_rect(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        let x1 =
            Self::parse_expression(TokenType::NonTerm(rect.next().unwrap().into_inner())).unwrap();
        let y1 =
            Self::parse_expression(TokenType::NonTerm(rect.next().unwrap().into_inner())).unwrap();
        let x2 =
            Self::parse_expression(TokenType::NonTerm(rect.next().unwrap().into_inner())).unwrap();
        let y2 =
            Self::parse_expression(TokenType::NonTerm(rect.next().unwrap().into_inner())).unwrap();
        Some(RascalExpression::Query(GeometryQuery::Rect(
            Box::new(x1),
            Box::new(y1),
            Box::new(x2),
            Box::new(y2),
    ...
    fn parse_circle(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        let x =
            Self::parse_expression(TokenType::NonTerm(circ.next().unwrap().into_inner())).unwrap();
        let y =
            Self::parse_expression(TokenType::NonTerm(circ.next().unwrap().into_inner())).unwrap();
        let r =
            Self::parse_expression(TokenType::NonTerm(circ.next().unwrap().into_inner())).unwrap();
        Some(RascalExpression::Query(GeometryQuery::Circle(
            Box::new(x),
            Box::new(y),
            Box::new(r),
        )))
    ...
    fn parse_line(expr: Pair<Rule>) -> Option<RascalExpression> {
        let mut line = expr.into_inner();
        let x1 =
            Self::parse_expression(TokenType::NonTerm(line.next().unwrap().into_inner())).unwrap();
        let y1 =
            Self::parse_expression(TokenType::NonTerm(line.next().unwrap().into_inner())).unwrap();
        let x2 =
            Self::parse_expression(TokenType::NonTerm(line.next().unwrap().into_inner())).unwrap();
        let y2 =
            Self::parse_expression(TokenType::NonTerm(line.next().unwrap().into_inner())).unwrap();
        Some(RascalExpression::Query(GeometryQuery::Line(
            Box::new(x1),
            Box::new(y1),
            Box::new(x2),
            Box::new(y2),
    ...
    fn parse_union(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        Some(RascalExpression::Query(GeometryQuery::Union(
            Box::new(e1),
            Box::new(e2),
        )))
    }
    ...
    fn parse_intersect(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        Some(RascalExpression::Query(GeometryQuery::Intersect(
            Box::new(e1),
            Box::new(e2),
        )))
    }
    ...
    fn parse_diff(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        Some(RascalExpression::Query(GeometryQuery::Diff(
            Box::new(e1),
            Box::new(e2),
        )))
    }
    ...
    fn parse_shrink(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        Some(RascalExpression::Query(GeometryQuery::Shrink(
            Box::new(e1),
            Box::new(e2),
        )))
    }
    ...
    fn parse_expand(expr: Pair<Rule>) -> Option<RascalExpression> {
        ...
        Some(RascalExpression::Query(GeometryQuery::Expand(
            Box::new(e1),
            Box::new(e2),
        )))
    }
    ...
    fn parse_bottom_expr(pair: Pair<Rule>) -> Option<RascalExpression> {
        match pair.as_rule() {
            ...
            Rule::any_char => Some(RascalExpression::SymbolLiteral(
                pair.as_str().chars().next().unwrap() as u16,
            )),
            Rule::identifier | Rule::name => {
                Some(RascalExpression::Identifier(pair.as_str().to_string()))
            }
            Rule::expression => Self::parse_expression(TokenType::NonTerm(pair.into_inner())),
            Rule::complex_call_site => Some(RascalExpression::Tag(
                Self::parse_complex_call_site(pair).unwrap(),
            )),
            ...
            Rule::proc_reference => Some(RascalExpression::Ref(
    ...
    fn parse_infix_expr(
        lhs: Option<RascalExpression>,
        op: Pair<Rule>,
        rhs: Option<RascalExpression>,
    ) -> Option<RascalExpression> {
        let lhs = lhs.unwrap();
        let rhs = rhs.unwrap();
        match op.as_rule() {
            Rule::and_op => Some(RascalExpression::BoolOp(
                Box::new(lhs),
                BoolOper::And,
                Box::new(rhs),
            )),
            Rule::or_op => Some(RascalExpression::BoolOp(
                Box::new(lhs),
                BoolOper::Or,
                Box::new(rhs),
            )),

            Rule::eq_rel => Some(RascalExpression::Relation(
                Box::new(lhs),
                Rel::Eq,
                Box::new(rhs),
            )),
            Rule::ne_rel => Some(RascalExpression::Relation(
                Box::new(lhs),
                Rel::Ne,
                Box::new(rhs),
            )),
            Rule::lt_rel => Some(RascalExpression::Relation(
                Box::new(lhs),
                Rel::Lt,
                Box::new(rhs),
            )),
            Rule::gt_rel => Some(RascalExpression::Relation(
                Box::new(lhs),
                Rel::Gt,
                Box::new(rhs),
            )),
            Rule::le_rel => Some(RascalExpression::Relation(
                Box::new(lhs),
                Rel::Le,
                Box::new(rhs),
            )),
            Rule::ge_rel => Some(RascalExpression::Relation(
                Box::new(lhs),
                Rel::Ge,
                Box::new(rhs),
            )),

            Rule::plus_op => Some(RascalExpression::Binary(
                Box::new(lhs),
                Op::Add,
                Box::new(rhs),
            )),
            Rule::minus_op => Some(RascalExpression::Binary(
                Box::new(lhs),
                Op::Sub,
                Box::new(rhs),
            )),
            Rule::mul_op => Some(RascalExpression::Binary(
                Box::new(lhs),
                Op::Mul,
                Box::new(rhs),
            )),
            Rule::div_op => Some(RascalExpression::Binary(
                Box::new(lhs),
                Op::Div,
                Box::new(rhs),
            )),
            Rule::mod_op => Some(RascalExpression::Binary(
                Box::new(lhs),
                Op::Mod,
                Box::new(rhs),
            )),
    ...
    fn parse_expression(exprs: TokenType) -> Option<RascalExpression> {
        match exprs {
            TokenType::NonTerm(expr) => {
                EXPR_PREC_CLIMBER.climb(expr, Self::parse_bottom_expr, Self::parse_infix_expr)
            }

            TokenType::Term(pair) => Self::parse_bottom_expr(pair),
        }
    }
    ...
    fn parse_position_expression(
        rule: &mut Pairs<Rule>,
    ) -> Option<(RascalExpression, RascalExpression)> {
    ...
    fn parse_color_expression(
        kind: Rule,
        rule: &mut Pairs<Rule>,
    ) -> Option<(RascalExpression, RascalExpression, RascalExpression)> {
        match rule.peek() {
            Some(r) if r.as_rule() == kind => {
                let mut fg_rule = rule.next().unwrap().into_inner();
                let h =
                    Self::parse_expression(NonTerm(fg_rule.next().unwrap().into_inner())).unwrap();
                let s =
                    Self::parse_expression(NonTerm(fg_rule.next().unwrap().into_inner())).unwrap();
                let v =
                    Self::parse_expression(NonTerm(fg_rule.next().unwrap().into_inner())).unwrap();
    ...
    fn parse_if_statement(statement: Pair<Rule>) -> RascalBlock {
        ...
        if let Some(guard) =
            Self::parse_expression(NonTerm(if_statement.next().unwrap().into_inner()))
        {
            let then_branch = Self::parse_block_body(if_statement.next().unwrap());
            result.push(RascalStatement::IfElse(
                guard,
                Box::new(then_branch),
                if_statement.next().map(|else_block| {
                    let else_piece = else_block.clone().into_inner().next().unwrap();
    ...
    fn parse_mod_assignment(statement: Pair<Rule>) -> RascalBlock {
        ...
        result.push(match assign.next().unwrap().as_str().trim() {
            "+=" => RascalStatement::Inc(
                lhs,
                Self::parse_expression(NonTerm(assign.next().unwrap().into_inner())).unwrap(),
            ),
            "-=" => RascalStatement::Dec(
                lhs,
                Self::parse_expression(NonTerm(assign.next().unwrap().into_inner())).unwrap(),
    ...
    fn parse_for_loop_statement(statement: Pair<Rule>) -> RascalBlock {
        ...
        {
            if let Some(from) =
                Self::parse_expression(TokenType::Term(for_statement.next().unwrap()))
            {
                if let Some(to) =
                    Self::parse_expression(TokenType::Term(for_statement.next().unwrap()))
                {
    ...
    fn parse_proc_call_statement(statement: Pair<Rule>) -> RascalBlock {
        let mut result = Vec::new();
        let proc_call =
            Self::parse_proc_call(&mut statement.into_inner().next().unwrap().into_inner());
    ...
    fn parse_print_statement(statement: Pair<Rule>) -> RascalBlock {
        ...
        let position = Self::parse_position_expression(&mut print_statement).unwrap_or((
            RascalExpression::NumLiteral(0),
            RascalExpression::NumLiteral(0),
        ));

        let foreground = Self::parse_color_expression(Rule::foreground, &mut print_statement)
            .unwrap_or((
                RascalExpression::NumLiteral(0),
                RascalExpression::NumLiteral(0),
                RascalExpression::NumLiteral(255),
            ));

        let background = Self::parse_color_expression(Rule::background, &mut print_statement)
            .unwrap_or((
                RascalExpression::NumLiteral(0),
                RascalExpression::NumLiteral(0),
                RascalExpression::NumLiteral(0),
        ...
        if let Some(expr) =
            Self::parse_expression(NonTerm(print_statement.next().unwrap().into_inner()))
        {
            result.push(RascalStatement::Print(
                position, foreground, background, expr,
            ));
        }
    ...
    fn parse_match_statement(statement: Pair<Rule>) -> RascalBlock {
        ...
        for case in match_statement {
            match case.as_rule() {
                Rule::then_block => {
                    then_block =
                        Some(Self::parse_block_body(case.into_inner().next().unwrap()).clone())
                }

                Rule::else_block => {
                    let inner_case = case.into_inner().next().unwrap();
                ...
                _ => {
                    ...
                    let case_pattern =
                        Self::parse_expression(TokenType::Term(case.next().unwrap())).unwrap();
                    let case_block = {
    ...
    fn parse_statement(statement: Pair<Rule>) -> RascalBlock {
        ...
        match statement.as_rule() {
            ...
            Rule::spawn_statement | Rule::update_statement => {
                result.extend(Self::parse_spawn_or_update_statement(statement))
            }
            ...
            Rule::debug_print_statement => {
                result.extend(Self::parse_debug_print_statement(statement))
            }
...
pub fn parse_rascal(src: &str) -> Vec<RascalStruct> {
    fn parse_component_name_and_members(
        mut rule: pest::iterators::Pairs<Rule>,
    ) -> (&str, Vec<(&str, DataType)>) {
        ...
        let members: Vec<(&str, DataType)> = if let Some(_) = rule.peek() {
            rule.next()
                ...
                .map(|p| {
                    ...
                    let datatype_name =
                        DataType::parse_datatype(member_rule.next().unwrap().as_str());
    ...
    let program =
        RascalParser::parse(Rule::program, src.trim()).unwrap_or_else(|e| panic!("{}", e));
    ...
    for parse_tree in program {
        match parse_tree.as_rule() {
            Rule::state_decl => {
                let state_rule = parse_tree.into_inner();
            ...
            Rule::event_decl => {
                let event_rule = parse_tree.into_inner();
            ...
            Rule::system_decl => {
                let inner = parse_tree.into_inner();
            ...
            Rule::unique_decl => {
                ...
                match datatype.as_rule() {
                    Rule::field => {
                        ...
                        let datatype = DataType::parse_datatype(
                            datatype.into_inner().next().unwrap().as_str(),
                        );
                        let uniq =
                            RascalStruct::Unique(name, DataType::Field, Some(datatype), field);
                        ast.push(uniq);
                    }
                    ...
                    Rule::scalar_type => {
                        ...
                        let uniq =
                            RascalStruct::Unique(name, datatype.clone(), None, datatype.default());
                        ast.push(uniq);
                    }
